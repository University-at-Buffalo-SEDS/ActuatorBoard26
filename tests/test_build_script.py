...
class OtaBuildScriptTests(unittest.TestCase):
    ...
    def test_simulation_profiles_firmware_failure_counters(self):
        ...
        self.assertEqual(fault_devices, [])
        self.assertFalse(any(device.get("disconnect_after") for device in layout["peripherals"]))
    ...
    def test_docker_run_prefix_supports_restricted_hosts(self):
        ...
        with mock.patch.dict(run_full.os.environ, {}, clear=True):
            self.assertEqual(
                run_full.docker_run_prefix("/usr/bin/docker"),
                ["/usr/bin/docker", "run", "--platform", "linux/amd64", "--rm"],
            )
        with mock.patch.dict(
            ...
            self.assertEqual(
                run_full.docker_run_prefix("/usr/bin/docker")[-2:],
                ["--network", "host"],
            )
