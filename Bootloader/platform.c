...
{
    const uint32_t app_end = ACTUATOR_SLOT_A_BASE + ACTUATOR_SLOT_A_SIZE;
    const bool vector_ok = vector_table_addr == ACTUATOR_VECTOR_TABLE;
    ...
    const bool reset_ok = reset_handler >= ACTUATOR_VECTOR_TABLE && reset_handler < app_end &&
                          (reset_handler & 1u) != 0u;
