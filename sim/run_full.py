    ...
    *,
    ...
    topology = {
        ...
        "reboots": [
            ...
            # Restart the actual host process while retaining its on-disk
            # variable cache. The preferred-master request must rebuild the
            # complete named graph without a network-wide discovery reset.
        ...
        "assertions": [
            {"name": "RF produced its simulated 1 Hz GPS stream", "node": "rf", "probe": "gps_publish_ok", "minimum": 3},
            {"name": "Flight produced its simulated 1 Hz IMU stream", "node": "flight", "probe": "imu_publish_ok", "minimum": 3},
            {"name": "Power produced its five-second voltage stream", "node": "power", "probe": "voltage_publish_ok", "minimum": 2},
